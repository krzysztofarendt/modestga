...
class TestModestga(unittest.TestCase):
    ...
    def test_individual(self):
        ...
        self.assertTrue((np.abs(np.array([0, 2.5, 5.0, 7.5, 10.0]) - est)).sum() < 1e-8)
    ...
    def test_ga(self):
        ...
        opt3 = minimize(self.fun, bounds, x0=x0, options=options, workers=1)

        # Test pop_size and trm_size re-adjusting
        options = {"generations": 3, "pop_size": 8}
        opt4 = minimize(self.fun, bounds, x0=x0, options=options, workers=2)
    ...
    def test_ga_1param(self):
        ...
        options = {"generations": 3, "pop_size": 4, "trm_size": 1}
        res = minimize(self.fun, bounds, x0=x0, options=options, workers=1)
    ...
    def test_exotic_pickling(self):
        ...
        res = minimize(
            fun_to_pickle, bounds, x0=x0, args=args, options=options, workers=2
        )
